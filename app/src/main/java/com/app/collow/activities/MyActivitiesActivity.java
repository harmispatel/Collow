...
public class MyActivitiesActivity extends BaseActivity implements SetupViewInterface {
    ...
    public static MyActivitiesActivity myActivitiesActivity = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            retryParameterbean = new RetryParameterbean(MyActivitiesActivity.this, getApplicationContext(), getIntent().getExtras(), MyActivitiesActivity.class.getClass());
            myActivitiesActivity = this;
            commonSession = new CommonSession(MyActivitiesActivity.this);
        ...
        try {
            ...
            imageView_right_menu.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (screen_from_index == ScreensEnums.MY_FOLLOWING_LISTING.getScrenIndex()) {
                        ...
                        MyUtils.openCommunityMenu(MyActivitiesActivity.this, requestParametersbean.getCommunityID(), requestParametersbean.getCommunityText(), communityAccessbean, true);
                        ...
                        MyUtils.openCommunityMenu(MyActivitiesActivity.this, loginbean.getHomeCommunityId(), loginbean.getName(), communityAccessbean, true);
            ...
            DrawerLayout.DrawerListener drawerListener = new DrawerLayout.DrawerListener() {
                ...
                public void onDrawerOpened(View drawerView) {
                    drawerLayout.closeDrawer(Gravity.RIGHT);
                    MyUtils.leftMenuUpdateDataifOpenedDrawer(MyActivitiesActivity.this, drawerLayout, circularImageView_profile_pic, baseTextview_left_menu_unread_message, retryParameterbean);
                }
    ...
    private void findViewByIDs() {
        try {
            ...
            communityActivitiesFeedAdapter = new CommunityActivitiesFeedAdapter(mRecyclerView, MyActivitiesActivity.this,"activity");
    ...
    public void getMyActivities() {
        try {
            ...
            JSONObject jsonObjectGetPostParameterEachScreen = GetPostParameterEachScreen.getPostParametersAccordingIndex(ScreensEnums.MY_ACTIVITIES.getScrenIndex(), requestParametersbean);
            PassParameterbean passParameterbean = new PassParameterbean(this, MyActivitiesActivity.this, getApplicationContext(), URLs.GET_COMMUNITIES_FEEDS, jsonObjectGetPostParameterEachScreen, ScreensEnums.MY_ACTIVITIES.getScrenIndex(), SignInActivity.class.getClass());
                    ...
                    if (CommonMethods.checkJSONArrayHasData(jsonObject_main, JSONCommonKeywords.Activities)) {
                        ...
                        for (int i = 0; i < jsonArray_activities.length(); i++) {
                            ...
                            if (images_of_classified.size() == 0) {
                                ...
                                ImageSlideAdapter imageSlideAdapter = new ImageSlideAdapter(MyActivitiesActivity.this, images_of_classified, ScreensEnums.MY_ACTIVITIES.getScrenIndex());
...
public class MyActivitiesActivity extends BaseActivity implements SetupViewInterface {
    ...
    public static MyActivitiesActivity myActivitiesActivity = null;
    public static ArrayList<Activitiesbean> activitiesbeanArrayList = new ArrayList<>();
    protected Handler handler;
    View view_home = null;
    ...
    private void findViewByIDs() {
        try {
            ...
            myActivitiesAdapter = new MyActivitiesAdapter(MyActivitiesActivity.this, mRecyclerView);
            mRecyclerView.setAdapter(myActivitiesAdapter);
            frameLayout_container.addView(view_home);

            myActivitiesAdapter.setOnLoadMoreListener(new OnLoadMoreListener() {
                @Override
                public void onLoadMore() {
                    //add null , so the adapter will check view_type and show progress bar at bottom
    ...
    public void getMyActivities() {
        try {
            ...
            JSONObject jsonObjectGetPostParameterEachScreen = GetPostParameterEachScreen.getPostParametersAccordingIndex(ScreensEnums.MY_ACTIVITIES.getScrenIndex(), requestParametersbean);
            PassParameterbean passParameterbean = new PassParameterbean(this, MyActivitiesActivity.this, getApplicationContext(), URLs.MY_ACTIVITIES, jsonObjectGetPostParameterEachScreen, ScreensEnums.MY_ACTIVITIES.getScrenIndex(), SignInActivity.class.getClass());
        ...
        try {
            if (responcebean.isServiceSuccess()) {
                ...
                if (CommonMethods.checkSuccessResponceFromServer(jsonObject_main)) {
                    ...
                    if (CommonMethods.checkJSONArrayHasData(jsonObject_main, JSONCommonKeywords.MyActivities)) {

                        JSONArray jsonArray = jsonObject_main.getJSONArray(JSONCommonKeywords.MyActivities);
                        ArrayList<Activitiesbean> activitiesbeen_local = new ArrayList<>();
                        for (int i = 0; i < jsonArray.length(); i++) {

                            JSONObject jsonObject_single_activity = jsonArray.getJSONObject(i);
                            Activitiesbean activitiesbean = new Activitiesbean();

                            if (CommonMethods.handleKeyInJSON(jsonObject_single_activity, JSONCommonKeywords.ProfilePic)) {
                                activitiesbean.setProfilePic(jsonObject_single_activity.getString(JSONCommonKeywords.ProfilePic));
                            }

                            if (CommonMethods.handleKeyInJSON(jsonObject_single_activity, JSONCommonKeywords.UserName)) {
                                activitiesbean.setUserName(jsonObject_single_activity.getString(JSONCommonKeywords.UserName));
                            }

                            if (CommonMethods.handleKeyInJSON(jsonObject_single_activity, JSONCommonKeywords.Title)) {
                                activitiesbean.setTitle(jsonObject_single_activity.getString(JSONCommonKeywords.Title));
                            }

                            if (CommonMethods.handleKeyInJSON(jsonObject_single_activity, JSONCommonKeywords.Time)) {
                                String time = activitiesbean.getTime();
                                if (CommonMethods.isTextAvailable(time)) {
                                    String timenew = CommonMethods.dateConversionInAgo(MyActivitiesActivity.this, time);
                                    activitiesbean.setTime(timenew);

                                }
                            }

                            if (CommonMethods.handleKeyInJSON(jsonObject_single_activity, JSONCommonKeywords.CommunityID)) {
                                activitiesbean.setCommunityID(jsonObject_single_activity.getString(JSONCommonKeywords.CommunityID));
                            }

                            if (CommonMethods.handleKeyInJSON(jsonObject_single_activity, JSONCommonKeywords.CommunityName)) {
                                activitiesbean.setCommunityName(jsonObject_single_activity.getString(JSONCommonKeywords.CommunityName));
                            }

                            if (CommonMethods.handleKeyInJSON(jsonObject_single_activity, JSONCommonKeywords.CommentText)) {
                                activitiesbean.setCommentText(jsonObject_single_activity.getString(JSONCommonKeywords.CommentText));
                            }

                            if (CommonMethods.handleKeyInJSON(jsonObject_single_activity, JSONCommonKeywords.Type)) {
                                activitiesbean.setType(jsonObject_single_activity.getString(JSONCommonKeywords.Type));
                            }



                            activitiesbeen_local.add(activitiesbean);
                        ...
                        if (current_start == 0) {
                            activitiesbeanArrayList = activitiesbeen_local;
                            myActivitiesAdapter.notifyDataSetChanged();

                        } else {

                            int start = activitiesbeanArrayList.size();

                            for (int i = 0; i < activitiesbeen_local.size(); i++) {

                                activitiesbeanArrayList.add(start, activitiesbeen_local.get(i));
                                myActivitiesAdapter.notifyItemInserted(activitiesbeanArrayList.size());
                                start++;

                            }
                            myActivitiesAdapter.setLoaded();
